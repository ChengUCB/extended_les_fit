...
# user settings
# =========================
REF_XYZ = '/global/scratch/users/dongjinkim/potential_training/CACE-LES-dipole/data-sets/MAPbI3_BEC.xyz'
...
def plot_bec_result(folder_name, all_ref, all_pred, all_an, save_path):
    ...
    colors   = {1:"white", 8:"red", 6:"black", 7:"blue", 53:"purple", 82:"gray"}
    eltrans  = {1:"H",     8:"O",   7:"N",     6:"C", 53:"I", 82:"Pb"}
    el_list  = [ 1, 82, 7, 6, 53, 8]
    ...
    unique_an = np.unique(all_an)
    present_els = [el for el in el_list if el in unique_an]
    
    for el in present_els:
        if el == 1:
...
# main loop
# =========================
#subdirs = sorted([d for d in os.listdir('.') if os.path.isdir(d) and 'uiu' in d])
