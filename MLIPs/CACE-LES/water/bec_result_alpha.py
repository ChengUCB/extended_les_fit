...
TARGET_FILENAME = 'h2o_bec_test_cace.xyz'
...
def load_bec_data(ref_xyz_path, pred_xyz_path, epsilon_infty, parity, two_dim, use_alpha, apply_transpose, epsilon_0):
    ...
    for xyz in pred_xyz:
        les_bec = xyz.get_array('LES_BEC')
        
        if two_dim:
            bec_pred = les_bec[:, :9] + les_bec[:, 9:18]
        else:
            bec_pred = les_bec[:, :9] if les_bec.shape[1] > 9 else les_bec

        if use_alpha:
            volume = xyz.get_volume()
            if 'alphas' in xyz.arrays:
                alpha_arr = xyz.arrays['alphas']
...
subdirs = sorted([d for d in os.listdir('.') if os.path.isdir(d) and 'uiqiu'in d])
